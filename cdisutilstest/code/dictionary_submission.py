...
def put_entity_from_file(
    client, path, submitter, submission_path, data_dir, validate=True
):
    with open(os.path.join(data_dir, path), "r") as f:
        entity = f.read()
    print("entity", entity)
    r = client.put(
        submission_path, headers=submitter(submission_path, "put"), data=entity
    )
    if validate:
        assert r.status_code == 200, r.data
    return r
...
def project_creation_endpoint_helper(
    client, pg_driver, submitter, program, project, data_dir
):
    ...
    with pg_driver.session_scope():
        ...
        assert (
            pg_driver.nodes(md.Project).path("programs").props(name=program).count()
            == 1
        )
...
def put_entity_creation_valid_helper(
    ...
    entities = [
        ...
        if (
            not entity.startswith("_")
            and entity.endswith(".yaml")
            and entity != "program.yaml"
            and entity != "project.yaml"
            and entity != "metaschema.yaml"
        )
    ]
    yamls = [
        yaml.load(open(os.path.join(schema_path, entity)).read()) for entity in entities
    ]
    ...
    while len(search_q) != 0:
        ...
        for i in range(len(yamls)):
            for j in range(len(yamls[i]["links"])):
                if (
                    "name" in yamls[i]["links"][j]
                    and yamls[i]["links"][j]["name"] == parent
                    and yamls[i]["links"][j]["backref"] not in request_q
                    and check_all_parents_in_request_q(yamls[i]["links"])
                ):
                ...
                if "subgroup" in yamls[i]["links"][j]:
                    for subgroup in yamls[i]["links"][j]["subgroup"]:
                        if (
                            subgroup["name"] == parent
                            and subgroup["backref"] not in request_q
                            and check_all_parents_in_request_q(yamls[i]["links"])
                        ):
...
def put_entity_creation_invalid_helper(
    client, pg_driver, submitter, program, project, data_dir, invalid_data_dir
):
    put_program_project(client, program, project, data_dir, auth=submitter)
    invalid_jsons = os.listdir(invalid_data_dir)
    for invalid in invalid_jsons:
        r = put_entity_from_file(
            client,
            invalid,
            submitter,
            "v0/submission/" + program + "/" + project,
